...
|---------|-------------|-------|--------|
| **[Internal Document RAG & Video-to-Bilingual Work Instruction Agent](#)** | AI-powered enterprise knowledge and work-instruction system combining multilingual document RAG with video understanding. Technical documents are ingested with PyMuPDF, chunked and embedded using BGE-M3, and retrieved through ChromaDB + BM25 + Reciprocal Rank Fusion. Manufacturing videos are processed with Faster-Whisper, PySceneDetect, OpenCV and EasyOCR to identify and ground work steps against retrieved document evidence. A rule + LLM hybrid conflict detector flags discrepancies, while human confirmation and SHA-256 audit trails provide controlled verification. The pipeline generates structured bilingual English/Japanese SOPs with schema validation and evidence traceability, supported by an R&D validation framework covering models, timings, failures and test results | Python · Faster-Whisper · Groq LLM · BGE-M3 · ChromaDB · BM25/RRF · OpenCV · FFmpeg · EasyOCR · PyMuPDF | 🔬 R&D / In Development |
